...
function get_pg_connection(): PgSql\Connection {
    ...
    $connStr = "host=localhost port=5432 dbname=scholarpath_db user=postgres password=bakmi2 options='--client_encoding=UTF8'";
    $conn = @pg_connect($connStr);

    if (!$conn) {
        // Jangan pakai @ di produksi; untuk debug bisa tampilkan detail
        $err = pg_last_error($conn) ?: 'Unknown error from pg_connect';
